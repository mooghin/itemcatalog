...
def makeAPIToken():
    ...
    # BELOW THIS LINE - copied from gconnect - verifies one-time code
    ...
    if result['user_id'] != gplus_id:
        response = make_response(json.dumps(
                                "Token's user ID doesn't match request."),
                                401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    if result['issued_to'] != CLIENT_ID:
        response = make_response(json.dumps(
                                "Token's client ID doesn't match app."), 401)
        response.headers['Content-Type'] = 'application/json'
        return response

    # ABOVE THIS LINE: COPIED FROM gconnect - verifying one-time code
    ...
    # If this user has never requested access before,
    # record them in our database
    user = session.query(User).filter_by(email=data['email']).first()
    if not user:
        user = User(username=data['name'], picture=data['picture'],
                    email=data['email'])
        session.add(user)
        session.commit()
...
def gconnect():
    ...
    print("Logged in " + login_session['username'] + "(" +
          login_session['email'] + ").")
    return output
...
def gdisconnect():
    ...
    # Since credentials was just stored as a string in login_session,
    ...
    # Response code 200: Revoking token successful.
...
def showRecentItems():
    ...
    topten = session.query(Item).order_by(desc(Item.date_added)).\
        limit(10).all()
...
def showFullCategory(category_name):
    ...
    # Show the template with new item capability if logged in
        ...
        return render_template('show_category_protected.html',
...
def categoryJSON(category_name):
    ...
    if category is not None:
        ...
        return jsonify({'error': 'Category %s does not exist.' %
                       category_name})
...
def itemJSON(category_name, item_name):
    ...
    if category is None:
        ...
        if item is None:
            return jsonify({'error': 'Item %s does not exist in category %s.' %
...
def apiNewItem():
    ...
    if category is None:
        return jsonify({'error': 'Cannot add item; \
...
def apiEditItem():
    ...
    if category_name is None or item_name is None:
        return jsonify({'error': 'Calls to /api/edit_item require \
                       2 parameters: category and name.'})

    category = session.query(Category).filter_by(name=category_name).first()

    if category is None:
        return jsonify({'error': 'Cannot edit item; Category %s does not \
                        exist.' % category_name})
    ...
    if new_category_name is not None:
        ...
        if new_category is None:
            return jsonify({'error': 'Cannot edit item; new category %s does \
                            not exist.' % new_category_name})
        else:
            item.category = new_category
...
def apiDeleteItem():
    ...
    if category is None:
        return jsonify({'error': 'Cannot delete item; Category %s does not \
                        exist.' % category_name})
